...
import {
  IAssetInventoriedDept,
  IAssetInventory,
  IAssetInventoryAssetTemporary,
  IAssetTypeInterface,
  IAudit,
  ICommitee,
} from 'interface';
...
const AuditDetail = () => {
  ...
  const [inventoryTemporaryLines, setInventoryTemporaryLines] = useState<
    IAssetInventoryAssetTemporary[]
  >([]);
  ...
  const handleGetAudit = async () => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-audit/${id}`);
      if (data.length > 0) {
        setAuditData(data[0]);
      }
    } catch (error) {
  ...
  const handleGetInventoryCommitte = async () => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-asset-inventory-commitee/${id}`);
      if (data.length > 0) {
        setCommitee(data);
      }
    } catch (error) {
  ...
  const handleGetAssetTypes = async () => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-asset-types`);
      if (data.length > 0) {
        setAssetTypes(data);
      }
    } catch (error) {
  ...
  const handleGetInventoriedDept = async () => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-asset-inventoried-dept/${id}`);
      if (data.length > 0) {
  ...
  const handleGetAssetInventoryLines = async (noLoading?: boolean) => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-asset-inventory/${id}`);
      if (data.length > 0) {
  ...
  const handleCheckIfUserIsAssigned = async () => {
    try {
      setLoading(true);
      const {
        data: { isAssigned, assignedLineId, myTemporaryId },
      } = await app.get(`/api/check-if-user-is-assigned?asset_inventory_id=${id}`);
  ...
  const handleOpenInventoryLine = (id: string) => {
    const inventoriedAsset = [...inventoryLines].find(
      (i) => (i.asset_id as any)[0].toString() === id.toString()
    );
    if (inventoriedAsset) {
      setActive(2);
  ...
  const handleGetAssetInventoryAssetTemporaryLines = async (noLoading?: boolean) => {
    try {
      ...
      const {
        data: { data },
      } = await app.get(`/api/get-asset-inventory-asset-temporary-lines/${id}`);
      if (data.length > 0) {
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            <IoArrowBackSharp
              ...
              onClick={() => navigate('/asset/audit', { replace: true })}
            />
          </div>
          <h5 style={{ margin: 0, fontSize: 14, color: 'white', fontWeight: 500 }}>
            Thông tin chi tiết
          </h5>
          {inventoryLines.length > 0 && auditData?.state === 'process' && (
            <div
              style={{
                display: 'flex',
                justifyContent: 'flex-start',
        ...
        >
          ...
          >
            <span
              style={{
                fontSize: 13,
                color: `${active === 1 ? myColor.buttonColor : 'grey'}`,
          ...
          >
            <span
              style={{
                ...
                fontSize: 13,
                color: `${active === 2 ? myColor.buttonColor : 'grey'}`,
              }}
            >
              Danh sách kiểm kê
            </span>
          ...
          >
            <span
              style={{
                ...
                fontSize: 13,
                color: `${active === 3 ? myColor.buttonColor : 'grey'}`,
              }}
            >
              Phát sinh
            </span>
      ...
      <div
        style={{ padding: '0.5rem 0rem', background: myColor.backgroundColor, overflow: 'auto' }}
      >
        {handleViewContent()}
      </div>
      {isOpen && (
        <QRScanner isOpen={isOpen} setOpen={setOpen} setDecodedText={handleOpenInventoryLine} />
      )}
    </div>
  );
};
