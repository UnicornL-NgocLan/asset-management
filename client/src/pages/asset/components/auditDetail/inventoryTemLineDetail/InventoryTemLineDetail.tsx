...
const InventoryTemLineDetail = ({
  ...
  const handleGetData = async () => {
    try {
      setLoading(true)
      await Promise.all([handleGetEmployeeTemporary(), handleGetInventoryLine(), handleGetUoms(), handleGetCompanies()]);
    } catch (error) {
      const message = getErrorMessage(error);
      alert(message);
    } finally {
  ...
  const onFinish = async () => {
    try {
      setIsUpdating(true);
      const { name, code, description, uom_id, company_using, company_owner, tt, note, de_xuat_xu_ly, giai_trinh } = form.getFieldsValue([
        "tt",
        "note",
        "de_xuat_xu_ly",
        "giai_trinh",
  ...
  return (
    <div style={{ position: "fixed", top: 0, width: "100vw", overflow: "auto", height: "100vh", zIndex: 100, background: myColor.backgroundColor }}>
      ...
      >
        ...
        <h5 style={{ margin: 0, fontSize: 14, color: "white", fontWeight: 500 }}>{openEdit?.id ? "Thông tin chi tiết" : "Tạo mới phát sinh"}</h5>
      </header>
          ...
          <div style={{ background: "white", padding: "1rem 1rem", borderRadius: 5, boxShadow: "2px 2px 2px rgba(0,0,0,0.2)" }}>
            <div style={{ paddingBottom: 10 }}>
              ...
              <Form.Item name="name" required className="m-0" style={{ margin: 0 }}>
                <Input placeholder="Ghế xoay..." size="middle" style={{ fontSize: 13, background: myColor.backgroundColor }} />
              </Form.Item>
            </div>
            <div style={{ paddingBottom: 10 }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Mã</p>
              <Form.Item name="code" className="m-0" style={{ margin: 0 }}>
                <Input size="middle" style={{ fontSize: 13, background: myColor.backgroundColor }} />
              </Form.Item>
            </div>
            <div style={{ paddingBottom: 10 }}>
              ...
              <Form.Item name="description" className="m-0" style={{ margin: 0 }}>
                <TextArea autoSize={{ minRows: 2, maxRows: 5 }} size="middle" style={{ fontSize: 13, background: myColor.backgroundColor }} />
              </Form.Item>
            </div>
            <div style={{ paddingBottom: 10 }}>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
