...
const Info = ({isCurrentUserAssigned,assignedLineId,assetTypes,auditData,commitee,inventoriedDept,handleGetData}
    ...
    const handleShowAuditLocation = (idList:number[]) => {
        ...
        const officeList = [...idList].map((id,index)=>{
            ...
            return <Tag style={{margin:0,fontSize:10}} key={index}>{office?.name}</Tag>
        })
    ...
    const handleShowAuditDepartments = (idList:number[]) => {
        ...
        const depLists = [...idList].map((id,index)=>{
            ...
            return <Tag style={{margin:0,fontSize:10}} key={index}>{dept?.name}</Tag>
        })
    ...
    const handleChangeToVerifyState = async () => {
        try {
            if(window.confirm("Bạn có chắc muốn kết thúc giai đoạn kiểm kê và chuyển sang giai đoạn xác nhận?")){
                if(isProcessing) return;
                setIsProcessing(true)
        ...
        <div style={{background:'white',padding:'0.5rem 1rem', borderRadius:5,boxShadow:'2px 2px 2px rgba(0,0,0,0.2)'}}>
            ...
            <div>
                <div>
                    ...
                    <p style={{fontSize:12,margin:'0.5rem 0', display:'flex',alignItems:'center',gap:5, flexWrap:'wrap'}}><span style={{fontWeight:500}}>Loại tài sản: </span> {handleShowAssetTypeAudited(auditData.asset_type_ids)}</p>
