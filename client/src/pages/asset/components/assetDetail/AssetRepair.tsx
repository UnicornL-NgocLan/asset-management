                ...
                <div>
                  <div>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      ...
                      {item.repair_date_start &&
                        moment(item.repair_date_start).add(7, 'hours').format('DD-MM-YYYY')}
                    </p>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      <span style={{ fontWeight: 500 }}>Ngày kết thúc: </span>{' '}
                      {item.repair_date_end &&
                        moment(item.repair_date_end).add(7, 'hours').format('DD-MM-YYYY')}
                    </p>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      <span style={{ fontWeight: 500 }}>Số lượng: </span> {item.quantity}
                    </p>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      <span style={{ fontWeight: 500 }}>Đơn vị sửa chữa:</span> {item.repair_party}
                    </p>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      <span style={{ fontWeight: 500 }}>Nơi xảy ra sự có:</span>{' '}
                      {item.accident_place}
                    </p>
                    <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                      <span style={{ fontWeight: 500 }}>Miêu tả:</span> {item.description}
                    </p>
                  </div>
                </div>
              </div>
