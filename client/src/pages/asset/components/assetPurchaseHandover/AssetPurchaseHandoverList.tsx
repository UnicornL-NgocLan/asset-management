...
import { IAssetTransfer, IPurchaseHandover } from 'interface';
...
const AssetPurchaseHandoverList = () => {
  const [data, setData] = useState<IPurchaseHandover[]>([]);
  ...
  const handleGetPurchaseHandoverList = async () => {
    try {
      setLoading(true);
      const {
        data: { data },
      } = await app.get('/api/get-purchase-handover-list');
      ...
      {loading ? (
        ...
        <Table
          ...
          dataSource={data.map((item) => ({
            ...item,
            receiver_employee: item.receive_party_id ? item.receive_party_id[1] : '',
            handover_employee: item.handover_party_id ? item.handover_party_id[1] : '',
            receiver_employee_id: item.receive_party_id ? item.receive_party_id[0] : null,
            handover_employee_id: item.handover_party_id ? item.handover_party_id[0] : null,
          }))}
          size="small"
          locale={locale}
          bordered
          ...
          onRow={(record) => {
            return {
              onClick: (_) => {
                navigate(`/asset/purchase-handover/${record.id}`);
              },
            };
          }}
...
export default AssetPurchaseHandoverList;
