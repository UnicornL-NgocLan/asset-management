...
import { IAssetTransferDetail, IAssetTransferLine } from 'interface/index.tsx';
...
import AssetTransferLineDetail from './AssetTransferLineDetail.tsx';

const AssetTransferDetail = () => {
  const [loading, setLoading] = useState(true);
  const [assetTransferData, setAssetTransferData] = useState<IAssetTransferDetail | null>(null);
  const [assetTransferLines, setAssetTransferLines] = useState<IAssetTransferLine[]>([]);
  const [isModalTransferLineOpen, setIsModalTransferLineOpen] = useState<IAssetTransferLine | null>(
    null
  );
  ...
  const handleGetAssetTransferDetail = async () => {
    try {
      setLoading(true);
      const {
        data: { data },
      } = await app.get('/api/get-asset-transfer-by-id/' + id);
      if (data.length > 0) {
        setAssetTransferData(data[0]);
        let roleInvovled = [
          {
            id: 1,
            role: 'Người bàn giao',
            employee_id_temp: data[0]?.handover_employee_id
              ? data[0].handover_employee_id[1]
              : null,
            confirm_completed: data[0]?.handover_already_confirm,
          },
          {
            id: 2,
            role: 'Người nhận bàn giao',
            employee_id_temp: data[0]?.receiver_employee_id
              ? data[0].receiver_employee_id[1]
              : null,
        ...
        const apiCalls = [
          app.get(`/api/check-userid-by-hr-temp-id?hr_temp_id=${data[0]?.handover_employee_id[0]}`),
          app.get(`/api/check-userid-by-hr-temp-id?hr_temp_id=${data[0]?.receiver_employee_id[0]}`),
        ];
  ...
  const handleGetAssetTransferLines = async () => {
    try {
      setLoading(true);
      const {
        data: { data },
      } = await app.get('/api/get-asset-transfer-line/' + id);
      setAssetTransferLines(data);
    } catch (error) {
      const message = getErrorMessage(error);
      alert(message);
    } finally {
  ...
  const handleConfirm = async (functionName: string) => {
    try {
      ...
      if (
        ...
        functionName.includes('action_handover_confirm')
      ) {
      ...
      await app.post('/api/transfer-handover-confirm', {
        uid: id,
        functionName: functionName,
        lat,
        lng,
      });
      await handleGetAssetTransferDetail();
    } catch (error) {
      const message = getErrorMessage(error);
      alert(message);
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
          <IoArrowBackSharp
            ...
            onClick={() => navigate('/asset/transfer', { replace: true })}
          />
        </div>
        <h5 style={{ margin: 0, fontSize: 14, color: 'white', fontWeight: 500 }}>
          {assetTransferData ? assetTransferData.name : 'Chi tiết phiếu chuyển tài sản'}
        </h5>
        <div></div>
      </header>
          ...
          <>
              ...
              <div>
                <div>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Mã phiếu:</span> {assetTransferData?.name}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Lý do:</span>{' '}
                    {assetTransferData?.transfer_reason}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Phiếu tổng hợp:</span>{' '}
                    {assetTransferData?.asset_request_assessment_id
                      ? assetTransferData.asset_request_assessment_id[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Văn bản trình ký:</span>{' '}
                    {assetTransferData?.sign_document_id
                      ? assetTransferData.sign_document_id[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Công ty sở hữu:</span>{' '}
                    {assetTransferData?.owner_company_id
                      ? assetTransferData.owner_company_id[1]
                      : ''}
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <div>
                <div>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Địa điểm hiện tại: </span>{' '}
                    {assetTransferData?.source_location_id
                      ? assetTransferData.source_location_id[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Phòng ban hiện tại: </span>{' '}
                    {assetTransferData?.source_department_temporary_id
                      ? assetTransferData.source_department_temporary_id[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Công ty đang sử dụng: </span>{' '}
                    {assetTransferData?.source_company_id
                      ? assetTransferData.source_company_id[1]
                      : ''}
                  </p>
                </div>
              </div>
            ...
            >
              ...
              <div>
                <div>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Địa điểm đích: </span>{' '}
                    {assetTransferData?.dest_location_id
                      ? assetTransferData.dest_location_id[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Phòng ban đích: </span>{' '}
                    {assetTransferData?.dest_department_temporary
                      ? assetTransferData.dest_department_temporary[1]
                      : ''}
                  </p>
                  <p style={{ fontSize: 12, margin: '0.5rem 0' }}>
                    <span style={{ fontWeight: 500 }}>Công ty đích: </span>{' '}
                    {assetTransferData?.dest_company_id ? assetTransferData.dest_company_id[1] : ''}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                {[
                  assetTransferData?.handover_confirm_latitude,
                  assetTransferData?.receiver_confirm_latitude,
                  assetTransferData?.handover_confirm_longitude,
                  assetTransferData?.receiver_confirm_longitude,
            ...
            >
              ...
              <div>
                <List
                  ...
                  dataSource={assetTransferLines}
                  renderItem={(item: IAssetTransferLine, index) => (
                    <List.Item
                      onClick={() => setIsModalTransferLineOpen(item)}
                      key={item.id}
                      style={{
                        display: 'block',
                        background: 'white',
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        ...
                        >
                          ...
                          <span style={{ margin: 0, fontSize: 12 }}>{item.quantity_done}</span>
                        </span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        <span
                        ...
                        >
                          <span style={{ fontWeight: 500 }}>Phòng ban nhận: </span>
                          <span style={{ margin: 0, fontSize: 12 }}>
                            {item.dest_department_temporary
                              ? item.dest_department_temporary[1]
                              : ''}
                          </span>
                        </span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        ...
                        >
                          <span style={{ fontWeight: 500 }}>Người sử dụng: </span>
                          <span style={{ margin: 0, fontSize: 12 }}>
                            {item.dest_asset_user_temporary
                              ? item.dest_asset_user_temporary[1]
                              : ''}
                          </span>
                        </span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        ...
                        >
                          <span style={{ fontWeight: 500 }}>Tình trạng khi điều chuyển: </span>
                          <span style={{ margin: 0, fontSize: 12 }}>
                            {vietNamTranslateState(item.asset_status_transfer)}
                          </span>
                        </span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            ...
            <div>
              {assetTransferData?.state === 'done' && isHandover && (
                <Button
                  onClick={() => handleConfirm('set_state_validated')}
                  size="large"
            ...
            <div>
              {assetTransferData?.state === 'done' && isHandover ? (
                !assetTransferData.handover_already_confirm ? (
                  <Button
                    onClick={() => handleConfirm('action_handover_confirm')}
                    size="large"
                    loading={isProcessing}
                    disabled={isProcessing}
                    style={{
                  ...
                  <Button
                    onClick={() => handleConfirm('action_handover_unconfirm')}
                    size="large"
                    loading={isProcessing}
                    disabled={isProcessing}
                    style={{
              ...
              {assetTransferData?.state === 'done' && isReceiver ? (
                !assetTransferData.receiver_already_confirm ? (
                  <Button
              ...
              {assetTransferData?.state === 'done' && isAssetManagement ? (
                !assetTransferData.asset_management_already_confirm ? (
                  <Button
              ...
              {assetTransferData?.state === 'done' && isRelatedParty ? (
                !assetTransferData.related_party_already_confirm ? (
                  <Button
              ...
              {[
                ...
                <div style={{ marginTop: 10 }}>
                  <img
                    ...
                    style={{ marginRight: 5 }}
                  />{' '}
                  Vị trí bên giao ({assetTransferData?.handover_confirm_latitude},{' '}
                  {assetTransferData?.handover_confirm_longitude})
                </div>
              )}
              {[
                ...
                <div style={{ marginTop: 10 }}>
                  <img
                    ...
                    style={{ marginRight: 5 }}
                  />{' '}
                  Vị trí bên nhận ({assetTransferData?.receiver_confirm_latitude},{' '}
                  {assetTransferData?.receiver_confirm_longitude})
                </div>
              )}
            </div>
      ...
      {isModalTransferLineOpen && (
        <AssetTransferLineDetail
          isModalTransferLineOpen={isModalTransferLineOpen}
          setIsModalTransferLineOpen={setIsModalTransferLineOpen}
          handleGetAssetTransferLines={handleGetAssetTransferLines}
          canEdit={
            assetTransferData?.state === 'done' &&
            isReceiver &&
            !assetTransferData.receiver_already_confirm
          }
        />
