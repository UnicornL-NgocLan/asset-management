...
const AuditList = ({ state }: { state: number }) => {
  ...
  const handleGetAuditList = async () => {
    try {
      ...
      const {
        data: { data },
      } = await app.get('/api/get-audit-list');
  ...
  const handleShowAuditLocation = (idList: number[]) => {
    ...
    const officeList = [...idList].map((id) => {
      ...
      return (
        <Tag style={{ margin: 0, fontSize: 10 }} key={office.id}>
          {office?.name}
        </Tag>
      );
    });
  ...
  useEffect(() => {
    switch (state) {
      ...
      case 1:
        setFilteredAudits(
          [...audits].filter((i) => i.state === 'draft').sort((a, b) => b.id - a.id)
        );
        break;
      case 2:
        setFilteredAudits(
          [...audits].filter((i) => i.state === 'process').sort((a, b) => b.id - a.id)
        );
        break;
      case 2.5:
        setFilteredAudits(
          [...audits].filter((i) => i.state === 'verifying').sort((a, b) => b.id - a.id)
        );
        break;
      case 3:
        setFilteredAudits(
          [...audits].filter((i) => i.state === 'validated').sort((a, b) => b.id - a.id)
        );
        break;
      case 4:
        setFilteredAudits(
          [...audits].filter((i) => i.state === 'cancel').sort((a, b) => b.id - a.id)
        );
        break;
      default:
        setFilteredAudits([...audits].sort((a, b) => b.id - a.id));
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                ...
                {item.start_time && item.end_time ? (
                  ...
                  >
                    ...
                    >{`Ngày bắt đầu: ${(item.start_time && moment(item.start_time).add(7, 'hours').format('DD-MM-YYYY HH:mm')) || ''}`}</span>
                  </span>
                ) : !item.start_time && item.end_time ? (
                  <span
                    style={{
                  ...
                  >
                    ...
                    <span
                      style={{ margin: 0, fontSize: 12 }}
                    >{`Ngày kết thúc: ${(item.end_time && moment(item.end_time).add(7, 'hours').format('DD-MM-YYYY HH:mm')) || ''}`}</span>
                  </span>
                ) : (
                  <span
                    style={{
                ...
                >
                  ...
                  {item.sea_office_id.length === 0 ? (
                    <span>Không có</span>
                  ) : (
                    handleShowAuditLocation(item.sea_office_id)
                  )}
                </span>
                {item.note && (
                  <span
