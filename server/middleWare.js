...
export const authenticateUser = async (req, res, next) => {
  try {
    ...
    if (mongoUser) {
      ...
      const uidSeaGroup = new Promise((resolve, reject) => {
        odooSeaGroup.connect((err, uid) => {
          if (err) return res.status(403).json({ msg: "Kết nối thất bại! Tên đăng nhập hoặc mật khẩu không đúng (Odoo Seagroup)" });
          return resolve(uid);
        });
      });

      const [isConnected, isConnectedSeagroup] = await Promise.all([uid, uidSeaGroup]);
      if (!isConnected || !isConnectedSeagroup)
        return res.status(403).json({ msg: "Kết nối với Odoo gặp trục trặc. Xin liên hệ với nhà phát triển" });
      const user = await getUserData(odoo, isConnected);
      if (!user) return res.status(403).json({ msg: "Dữ liệu người dùng không lấy được. Xin liên hệ với nhà phát triển" });
      req.user = user;
      req.odoo = odoo;
      req.odooSeaGroup = odooSeaGroup;
      return next();
    } else {
